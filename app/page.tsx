...
export default function Home() {
  ...
  const [dadosReserva, setDadosReserva] = useState({
    name: '', email: '', petName: '', size: 'PEQUENO', petsCount: 1,
  ...
  async function irParaConfirmacao(e: React.FormEvent<HTMLFormElement>) {
    ...
    const petsCount = parseInt(formData.get('petsCount') as string) || 1
    ...
    // O Motor agora recebe a quantidade de pets para multiplicar o valor
    const precoRes = await obterPrecoPreview(checkInDate, checkOutDate, petsCount)
    ...
    if (resultado.success) {
      const res = await fetch('/api/pagamento', {
        ...
        body: JSON.stringify({ 
          ...
          petName: `${dadosReserva.petsCount} Pet(s) - ${dadosReserva.petName}`
        }),
      });
          ...
          <div className="bg-white/70 backdrop-blur-xl border border-white/50 p-8 rounded-3xl shadow-2xl transition-all">
            ...
            {etapa === 'formulario' ? (
              <>
                ...
                <form onSubmit={irParaConfirmacao} className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <input required name="name" type="text" placeholder="Seu Nome Completo" className="w-full rounded-xl border-stone-200 bg-white/80 p-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 border shadow-sm" />
                    <input required name="email" type="email" placeholder="seu.email@exemplo.com" className="w-full rounded-xl border-stone-200 bg-white/80 p-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 border shadow-sm" />
                  </div>

                  <div className="grid grid-cols-3 gap-3">
                    <input required name="petName" type="text" placeholder="Nome do(s) Pet(s)" className="col-span-1 w-full rounded-xl border-stone-200 bg-white/80 p-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 border shadow-sm" />
                    
                    <select name="petsCount" className="col-span-1 w-full rounded-xl border-stone-200 bg-white/80 p-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 border shadow-sm text-stone-600">
                    ...
                    <select name="size" className="col-span-1 w-full rounded-xl border-stone-200 bg-white/80 p-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 border shadow-sm text-stone-600">
              ...
              <div className="space-y-6 animate-fadeIn">
                <div>
                  ...
                  <p className="text-stone-500 text-sm">Confirme as datas e horários antes de seguir para o pagamento.</p>
                </div>
                ...
                <div className="bg-white/90 rounded-2xl p-5 border border-stone-100 space-y-4 shadow-sm text-sm">
                  ...
                  <div className="grid grid-cols-3 gap-2 border-b border-stone-100 pb-3">
                    <div className="col-span-2"><span className="text-stone-400 block text-xs font-bold uppercase">Nome do(s) Pet(s)</span> <strong className="text-stone-800">{dadosReserva.petName}</strong></div>
                    <div><span className="text-stone-400 block text-xs font-bold uppercase">Qtd e Porte</span> <strong className="text-stone-800">{dadosReserva.petsCount} Pet(s) - {dadosReserva.size}</strong></div>
                  </div>
                ...
                <div className="bg-teal-50 border border-teal-100 rounded-2xl p-5 flex justify-between items-center shadow-sm">
                  <div>
                    ...
                    <span className="text-stone-500 text-xs">Calculado para {dadosReserva.petsCount} pet(s)</span>
                  </div>
